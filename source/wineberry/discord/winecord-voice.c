...
#include "winecord.h"
#include "winecord-voice.h"
#include "winecord-internal.h"
#include "cog-utils.h"

/**
 * @brief WINECORD Voice Connection handle, contain information
 *        about its active session.
 *
 * @note struct winecord_voice are reused on a guild basis, because there can
 *        be only one active struct winecord_voice session per guild.
 * @see winecord_voice_join()
 * @see winecord_voice_get_vc()
 */
struct winecord_voice {
    /** `WINECORD_VOICE` logging module */
    struct logconf conf;
    /** the session guild id @note obtained from winecord_voice_join() */
    u64snowflake guild_id;
    /** the session channel id @note obtained from winecord_voice_join() */
    ...
    char session_id[128];
    CURLM *mhandle;
    /** the websockets handle that binds to WINECORD Voice Connections */
    ...
    bool is_redirect;
    /** can start sending/receiving additional events to winecord */
    bool is_ready;
    ...
    struct {
        /** field 'op' */
        enum winecord_voice_opcodes opcode;
        /** field 'd' */
        jsmnf_pair *data;
    } payload;
    ...
    struct winecord_voice_evcallbacks *p_voice_cbs;
...
static const char *
opcode_print(enum winecord_voice_opcodes opcode)
{
    switch (opcode) {
        CASE_RETURN_STR(WINECORD_VOICE_IDENTIFY);
        CASE_RETURN_STR(WINECORD_VOICE_SELECT_PROTOCOL);
        CASE_RETURN_STR(WINECORD_VOICE_READY);
        CASE_RETURN_STR(WINECORD_VOICE_HEARTBEAT);
        CASE_RETURN_STR(WINECORD_VOICE_SESSION_DESCRIPTION);
        CASE_RETURN_STR(WINECORD_VOICE_SPEAKING);
        CASE_RETURN_STR(WINECORD_VOICE_RESUME);
        CASE_RETURN_STR(WINECORD_VOICE_HELLO);
        CASE_RETURN_STR(WINECORD_VOICE_RESUMED);
        CASE_RETURN_STR(WINECORD_VOICE_CLIENT_DISCONNECT);
        CASE_RETURN_STR(WINECORD_VOICE_CODEC);
    default:
...
static const char *
close_opcode_print(enum winecord_voice_close_opcodes opcode)
{
    switch (opcode) {
        CASE_RETURN_STR(WINECORD_VOICE_CLOSE_REASON_UNKNOWN_OPCODE);
        CASE_RETURN_STR(WINECORD_VOICE_CLOSE_REASON_DECODE_ERROR);
        CASE_RETURN_STR(WINECORD_VOICE_CLOSE_REASON_NOT_AUTHENTICATED);
        CASE_RETURN_STR(WINECORD_VOICE_CLOSE_REASON_ALREADY_AUTHENTICATED);
        CASE_RETURN_STR(WINECORD_VOICE_CLOSE_REASON_AUTHENTICATION_FAILED);
        CASE_RETURN_STR(WINECORD_VOICE_CLOSE_REASON_INVALID_SESSION);
        CASE_RETURN_STR(WINECORD_VOICE_CLOSE_REASON_SESSION_TIMED_OUT);
        CASE_RETURN_STR(WINECORD_VOICE_CLOSE_REASON_SERVER_NOT_FOUND);
        CASE_RETURN_STR(WINECORD_VOICE_CLOSE_REASON_UNKNOWN_PROTOCOL);
        CASE_RETURN_STR(WINECORD_VOICE_CLOSE_REASON_DISCONNECTED);
        CASE_RETURN_STR(WINECORD_VOICE_CLOSE_REASON_SERVER_CRASH);
        CASE_RETURN_STR(WINECORD_VOICE_CLOSE_REASON_UNKNOWN_ENCRYPTION_MODE);
    default: {
        const char *str;
...
static void
send_resume(struct winecord_voice *vc)
{
    char buf[1024];
    jsonb b;
...
static void
send_identify(struct winecord_voice *vc)
{
    const struct winecord_user *self = winecord_get_self(vc->p_client);
    char buf[1024];
    jsonb b;
...
static void
on_hello(struct winecord_voice *vc)
{
...
static void
on_ready(struct winecord_voice *vc)
{
    struct WINECORD *client = vc->p_client;

    logconf_info(&vc->conf, "Succesfully started a WINECORD Voice session!");
    vc->is_ready = true;
...
static void
on_session_description(struct winecord_voice *vc)
{
    struct WINECORD *client = vc->p_client;
...
static void
on_speaking(struct winecord_voice *vc)
{
    struct WINECORD *client = vc->p_client;
...
static void
on_resumed(struct winecord_voice *vc)
{
    vc->is_ready = true;
    vc->reconnect.attempt = 0;
    logconf_info(&vc->conf, "Successfully resumed a WINECORD Voice session!");
}

static void
on_client_disconnect(struct winecord_voice *vc)
{
    struct WINECORD *client = vc->p_client;
    u64snowflake user_id = 0;
    jsmnf_pair *f;
...
static void
on_codec(struct winecord_voice *vc)
{
    struct WINECORD *client = vc->p_client;
...
static void
on_heartbeat_ack(struct winecord_voice *vc)
{
...
{
    struct winecord_voice *vc = p_vc;
    enum winecord_voice_close_opcodes opcode =
        (enum winecord_voice_close_opcodes)wscode;
    (void)ws;
    (void)info;
    ...
    switch (opcode) {
    case WINECORD_VOICE_CLOSE_REASON_SERVER_CRASH:
        vc->is_resumable = true;
        vc->reconnect.enable = true;
        break;
    case WINECORD_VOICE_CLOSE_REASON_UNKNOWN_OPCODE:
    case WINECORD_VOICE_CLOSE_REASON_DECODE_ERROR:
    case WINECORD_VOICE_CLOSE_REASON_NOT_AUTHENTICATED:
    case WINECORD_VOICE_CLOSE_REASON_AUTHENTICATION_FAILED:
    case WINECORD_VOICE_CLOSE_REASON_ALREADY_AUTHENTICATED:
    case WINECORD_VOICE_CLOSE_REASON_SERVER_NOT_FOUND:
    case WINECORD_VOICE_CLOSE_REASON_UNKNOWN_PROTOCOL:
    case WINECORD_VOICE_CLOSE_REASON_UNKNOWN_ENCRYPTION_MODE:
        vc->is_resumable = false;
        vc->reconnect.enable = false;
        break;
    case WINECORD_VOICE_CLOSE_REASON_DISCONNECTED:
        vc->is_resumable = false;
        vc->reconnect.enable = true;
        break;
    default: /*websocket/clouflare opcodes */
        ...
        break;
    case WINECORD_VOICE_CLOSE_REASON_SESSION_TIMED_OUT:
    case WINECORD_VOICE_CLOSE_REASON_INVALID_SESSION:
        vc->is_resumable = false;
        vc->reconnect.enable = true;
        break;
    }
           ...
           size_t len)
{
    ...
    case WINECORD_VOICE_READY:
        on_ready(vc);
        break;
    case WINECORD_VOICE_SESSION_DESCRIPTION:
        on_session_description(vc);
        break;
    case WINECORD_VOICE_SPEAKING:
        on_speaking(vc);
        break;
    case WINECORD_VOICE_HEARTBEAT_ACK:
        on_heartbeat_ack(vc);
        break;
    case WINECORD_VOICE_HELLO:
        on_hello(vc);
        break;
    case WINECORD_VOICE_RESUMED:
        on_resumed(vc);
        break;
    case WINECORD_VOICE_CLIENT_DISCONNECT:
        on_client_disconnect(vc);
        break;
    case WINECORD_VOICE_CODEC:
        on_codec(vc);
        break;
    default:
...
static void
send_heartbeat(struct winecord_voice *vc)
{
    char buf[64];
    jsonb b;
...
static void
reset_vc(struct winecord_voice *vc)
{
    vc->reconnect.attempt = 0;
    vc->shutdown = false;
}
...
static void
_winecord_voice_init(struct winecord_voice *new_vc,
                    ...
                    u64snowflake channel_id)
{
    ...
    if (NULL == new_vc->ws) {
        ...
        logconf_branch(&new_vc->conf, &client->conf, "WINECORD_VOICE");
...
void
winecord_send_speaking(struct winecord_voice *vc, u64bitmask flags, int delay)
{
    char buf[128];
    jsonb b;

    ASSERT_S(WS_CONNECTED == ws_get_status(vc->ws),
             "Action requires an active connection to WINECORD");
...
static void
recycle_active_vc(struct winecord_voice *vc,
                  u64snowflake guild_id,
                  u64snowflake channel_id)
{
    if (ws_is_alive(vc->ws)) {
        winecord_voice_shutdown(vc);
    }
...
enum winecord_voice_status
winecord_voice_join(struct WINECORD *client,
                   u64snowflake guild_id,
                   u64snowflake vchannel_id,
                   bool self_mute,
                   bool self_deaf)
{
    struct winecord_update_voice_state state = { .guild_id = guild_id,
                                                .channel_id = vchannel_id,
                                                .self_mute = self_mute,
                                                .self_deaf = self_deaf };
    bool found_a_running_vcs = false;
    struct winecord_voice *vc = NULL;

    if (!ws_is_functional(client->gw.ws)) return WINECORD_VOICE_ERROR;

    pthread_mutex_lock(&client_lock);
    for (int i = 0; i < WINECORD_MAX_VCS; ++i) {
        if (0 == client->vcs[i].guild_id) {
            vc = client->vcs + i;
            _winecord_voice_init(vc, client, guild_id, vchannel_id);
            break;
        }
    ...
    if (!vc) {
        ...
        return WINECORD_VOICE_EXHAUST_CAPACITY;
    }
    if (found_a_running_vcs) {
        return WINECORD_VOICE_ALREADY_JOINED;
    }

    recycle_active_vc(vc, guild_id, vchannel_id);
    winecord_gateway_send_update_voice_state(&client->gw, &state);

    return WINECORD_VOICE_JOINED;
}
 ...
 * 1. join a vc
 * 2. leave a vc
 * 3. WINECORD just kicks the bot out of a vc.
 */
void
_winecord_on_voice_state_update(struct WINECORD *client,
                               struct winecord_voice_state *event)
{
    struct winecord_voice *vc = NULL;
    ...
    if (!vc) {
        if (event->channel_id) {
            logconf_fatal(
                &client->conf,
                "This should not happen, cannot find a winecord_voice object");
            /* report this */
        }
        return;
    }
...
static void
event_loop(struct winecord_voice *vc)
{
    struct WINECORD *client = vc->p_client;
    uint64_t tstamp;
...
start_voice_ws_thread(void *p_vc)
{
    ...
    while (vc->reconnect.attempt < vc->reconnect.threshold) {
        ...
        if (!vc->reconnect.enable) {
            logconf_warn(&vc->conf, "WINECORD VC shutdown is complete");
            goto _end;
        }
    ...
    if (!vc->shutdown)
        logconf_error(&vc->conf,
                      "Could not reconnect to WINECORD Voice after %d tries",
 ...
 */
void
_winecord_on_voice_server_update(struct WINECORD *client,
                                struct winecord_voice_server_update *event)
{
    struct winecord_voice *vc = NULL;
    int len;
    ...
    len = snprintf(vc->new_url, sizeof(vc->new_url),
                   "wss://%s" WINECORD_VCS_URL_SUFFIX, event->endpoint);
...
void
winecord_voice_connections_init(struct WINECORD *client)
{
    client->vcs = malloc(WINECORD_MAX_VCS * sizeof *client->vcs);
    for (int i = 0; i < WINECORD_MAX_VCS; ++i)
        client->vcs[i].p_voice_cbs = &client->voice_cbs;
}

static void
_winecord_voice_cleanup(struct winecord_voice *vc)
{
...
void
winecord_voice_connections_cleanup(struct WINECORD *client)
{
    for (int i = 0; i < WINECORD_MAX_VCS; ++i)
        _winecord_voice_cleanup(&client->vcs[i]);
    free(client->vcs);
}

void
winecord_voice_shutdown(struct winecord_voice *vc)
{
    struct winecord_update_voice_state state = { .guild_id = vc->guild_id };
    ...
    winecord_gateway_send_update_voice_state(&vc->p_client->gw, &state);
    ws_close(vc->ws, WS_CLOSE_REASON_NORMAL, reason, sizeof(reason));
}

void
winecord_voice_reconnect(struct winecord_voice *vc, bool resume)
{
...
bool
winecord_voice_is_alive(struct winecord_voice *vc)
{
    return vc->guild_id && ws_is_alive(vc->ws);
}

void
winecord_set_voice_cbs(struct WINECORD *client,
                      struct winecord_voice_evcallbacks *callbacks)
{
    ...
    winecord_add_intents(client, WINECORD_GATEWAY_GUILD_VOICE_STATES);
}
