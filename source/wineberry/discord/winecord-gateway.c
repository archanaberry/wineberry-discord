...
static const char *
_winecord_gateway_opcode_print(enum winecord_gateway_opcodes opcode)
{
    switch (opcode) {
        CASE_RETURN_STR(WINECORD_GATEWAY_DISPATCH);
        CASE_RETURN_STR(WINECORD_GATEWAY_HEARTBEAT);
        CASE_RETURN_STR(WINECORD_GATEWAY_IDENTIFY);
        CASE_RETURN_STR(WINECORD_GATEWAY_PRESENCE_UPDATE);
        CASE_RETURN_STR(WINECORD_GATEWAY_VOICE_STATE_UPDATE);
        CASE_RETURN_STR(WINECORD_GATEWAY_RESUME);
        CASE_RETURN_STR(WINECORD_GATEWAY_RECONNECT);
        CASE_RETURN_STR(WINECORD_GATEWAY_REQUEST_GUILD_MEMBERS);
        CASE_RETURN_STR(WINECORD_GATEWAY_INVALID_SESSION);
        CASE_RETURN_STR(WINECORD_GATEWAY_HELLO);
        CASE_RETURN_STR(WINECORD_GATEWAY_HEARTBEAT_ACK);
    default:
...
static const char *
_winecord_gateway_close_opcode_print(enum winecord_gateway_close_opcodes opcode)
{
    switch (opcode) {
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_UNKNOWN_ERROR);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_UNKNOWN_OPCODE);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_DECODE_ERROR);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_NOT_AUTHENTICATED);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_AUTHENTICATION_FAILED);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_ALREADY_AUTHENTICATED);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_INVALID_SEQUENCE);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_RATE_LIMITED);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_SESSION_TIMED_OUT);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_INVALID_SHARD);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_SHARDING_REQUIRED);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_INVALID_API_VERSION);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_INVALID_INTENTS);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_DISALLOWED_INTENTS);
        CASE_RETURN_STR(WINECORD_GATEWAY_CLOSE_REASON_RECONNECT);
    default: {
...
static void
_winecord_on_hello(struct winecord_gateway *gw)
{
    jsmnf_pair *f;
    ...
    if (gw->session->status & WINECORD_SESSION_RESUMABLE)
        winecord_gateway_send_resume(gw, &(struct winecord_resume){
                                            .token = gw->id.token,
                                            .session_id = gw->session->id,
                                            .seq = gw->payload.seq,
                                        });
    else
        winecord_gateway_send_identify(gw, &gw->id);
}

#define RETURN_IF_MATCH(event, str)                                           \
    if (!strcmp(#event, str)) return WINECORD_EV_##event

static enum winecord_gateway_events
_winecord_gateway_event_eval(const char name[])
{
    ...
    return WINECORD_EV_NONE;
}

#undef RETURN_IF_MATCH

static struct winecord_gateway *
_winecord_gateway_clone(const struct winecord_gateway *gw)
{
    return &winecord_clone(CLIENT(gw, gw))->gw;
}

static void
_winecord_gateway_clone_cleanup(struct winecord_gateway *clone)
{
    winecord_cleanup(CLIENT(clone, gw));
}

static void
_winecord_gateway_dispatch_thread(void *p_gw)
{
    struct winecord_gateway *gw = p_gw;

    logconf_info(&gw->conf,
                 "Thread " ANSICOLOR("starts", ANSI_FG_RED) " to serve %s",
                 gw->payload.name);

    winecord_gateway_dispatch(gw);

    logconf_info(&gw->conf,
                 "Thread " ANSICOLOR("exits", ANSI_FG_RED) " from serving %s",
                 gw->payload.name);

    _winecord_gateway_clone_cleanup(gw);
}

static void
_winecord_on_dispatch(struct winecord_gateway *gw)
{
  ...
  if (gw->timer->now - gw->timer->event_last < 60000) {
    ...
    case WINECORD_EV_READY: {
        jsmnf_pair *f;

        logconf_info(&gw->conf, "Successfully started a Winecord session!");
                            ...
                            "resume_gateway_url", 18)))
        {
            ...
            url_len = snprintf(gw->session->resume_url,
                               ...
                               "%.*s%s" WINECORD_GATEWAY_URL_SUFFIX, url_len,
                               url, ('/' == url[url_len - 1]) ? "" : "/");
    ...
    case WINECORD_EV_RESUMED:
        logconf_info(&gw->conf, "Succesfully resumed a Winecord session!");
    ...
    enum winecord_event_scheduler mode =
        gw->scheduler(client, gw->payload.json.start + gw->payload.data->v.pos,
                      gw->payload.data->v.len, gw->payload.event);

    switch (mode) {
    case WINECORD_EVENT_IGNORE:
        break;
    case WINECORD_EVENT_MAIN_THREAD:
        winecord_gateway_dispatch(gw);
        break;
    case WINECORD_EVENT_WORKER_THREAD: {
        struct winecord_gateway *clone = _winecord_gateway_clone(gw);
        WINEBERRY code = winecord_worker_add(
            client, &_winecord_gateway_dispatch_thread, clone);

        if (code != WINEBERRY_OK) {
            log_error("Couldn't start worker-thread (code %d)", code);
            _winecord_gateway_clone_cleanup(clone);
        }
    } break;
    default:
...
static void
_winecord_on_invalid_session(struct winecord_gateway *gw)
{
    ...
    gw->session->status = WINECORD_SESSION_SHUTDOWN;
    if (gw->payload.data->v.len != 5
        || strncmp("false", gw->payload.json.start + gw->payload.data->v.pos,
                   5))
    {
        gw->session->status |= WINECORD_SESSION_RESUMABLE;
        reason = "Invalid session, will attempt to resume";
        opcode = (enum ws_close_reason)WINECORD_GATEWAY_CLOSE_REASON_RECONNECT;
    }
    else {
...
static void
_winecord_on_reconnect(struct winecord_gateway *gw)
{
    const char reason[] = "Winecord expects client to reconnect";

    gw->session->status = WINECORD_SESSION_RESUMABLE | WINECORD_SESSION_SHUTDOWN;
    gw->session->retry.enable = true;

    ws_close(gw->ws,
             (enum ws_close_reason)WINECORD_GATEWAY_CLOSE_REASON_RECONNECT,
             reason, sizeof(reason));
    io_poller_curlm_enable_perform(CLIENT(gw, gw)->io_poller, gw->mhandle);
}

static void
_winecord_on_heartbeat_ack(struct winecord_gateway *gw)
{
             ...
             size_t len)
{
    ...
    (void)info;
    struct winecord_gateway *gw = p_gw;
    enum winecord_gateway_close_opcodes opcode =
        (enum winecord_gateway_close_opcodes)wscode;

    logconf_warn(
        &gw->conf,
        ANSICOLOR("CLOSE %s", ANSI_FG_RED) " (code: %4d, %zu bytes): '%.*s'",
        _winecord_gateway_close_opcode_print(opcode), opcode, len, (int)len,
        reason);

    /* user-triggered shutdown */
    if (gw->session->status & WINECORD_SESSION_SHUTDOWN) {
    ...
    default: /* websocket/clouflare opcodes */
        ...
        /* fall-through */
    case WINECORD_GATEWAY_CLOSE_REASON_INVALID_SEQUENCE:
    case WINECORD_GATEWAY_CLOSE_REASON_SESSION_TIMED_OUT:
        logconf_warn(
            &gw->conf,
        ...
        gw->session->retry.enable = true;
        break;
    case WINECORD_GATEWAY_CLOSE_REASON_AUTHENTICATION_FAILED:
    case WINECORD_GATEWAY_CLOSE_REASON_SHARDING_REQUIRED:
    case WINECORD_GATEWAY_CLOSE_REASON_INVALID_API_VERSION:
    case WINECORD_GATEWAY_CLOSE_REASON_INVALID_INTENTS:
    case WINECORD_GATEWAY_CLOSE_REASON_INVALID_SHARD:
    case WINECORD_GATEWAY_CLOSE_REASON_DISALLOWED_INTENTS:
        logconf_warn(&gw->conf, "Gateway will not attempt to reconnect");
        gw->session->status &= ~WINECORD_SESSION_RESUMABLE;
        gw->session->retry.enable = false;
        break;
    case WINECORD_GATEWAY_CLOSE_REASON_UNKNOWN_ERROR:
    case WINECORD_GATEWAY_CLOSE_REASON_UNKNOWN_OPCODE:
    case WINECORD_GATEWAY_CLOSE_REASON_DECODE_ERROR:
    case WINECORD_GATEWAY_CLOSE_REASON_NOT_AUTHENTICATED:
    case WINECORD_GATEWAY_CLOSE_REASON_ALREADY_AUTHENTICATED:
    case WINECORD_GATEWAY_CLOSE_REASON_RATE_LIMITED:
        logconf_warn(
            &gw->conf,
...
static bool
_winecord_gateway_payload_from_json(struct winecord_gateway_payload *payload,
                                   const char text[],
                                   size_t len)
{
    ...
    if ((f = jsmnf_find(payload->json.pairs, text, "t", 1))) {
        ...
        payload->event = _winecord_gateway_event_eval(payload->name);
    }
    ...
    if ((f = jsmnf_find(payload->json.pairs, text, "op", 2)))
        payload->opcode =
            (enum winecord_gateway_opcodes)strtol(text + f->v.pos, NULL, 10);
            ...
            size_t len)
{
    ...
    if (!_winecord_gateway_payload_from_json(&gw->payload, text, len)) {
    ...
    logconf_trace(
        ...
        _winecord_gateway_opcode_print(gw->payload.opcode),
        *gw->payload.name ? " -> " : "", gw->payload.name, len,
        info->loginfo.counter);

    switch (gw->payload.opcode) {
    case WINECORD_GATEWAY_DISPATCH:
        _winecord_on_dispatch(gw);
        break;
    case WINECORD_GATEWAY_INVALID_SESSION:
        _winecord_on_invalid_session(gw);
        break;
    case WINECORD_GATEWAY_RECONNECT:
        _winecord_on_reconnect(gw);
        break;
    case WINECORD_GATEWAY_HELLO:
        _winecord_on_hello(gw);
        break;
    case WINECORD_GATEWAY_HEARTBEAT_ACK:
        _winecord_on_heartbeat_ack(gw);
        break;
    default:
...
static winecord_event_scheduler_t
_winecord_on_scheduler_default(struct winecord *a,
                              const char b[],
                              size_t c,
                              enum winecord_gateway_events d)
{
    (void)a;
    (void)b;
    (void)c;
    (void)d;
    return WINECORD_EVENT_MAIN_THREAD;
}

static int
_winecord_on_gateway_perform(struct io_poller *io, CURLM *mhandle, void *p_gw)
{
    (void)io;
    (void)mhandle;
    return winecord_gateway_perform(p_gw);
}

void
winecord_gateway_init(struct winecord_gateway *gw,
                     struct logconf *conf,
                     const char token[])
{
    struct winecord *client = CLIENT(gw, gw);
    ...
    io_poller_curlm_add(client->io_poller, gw->mhandle,
                        _winecord_on_gateway_perform, gw);
    gw->ws = ws_init(&cbs, gw->mhandle, &attr);
    logconf_branch(&gw->conf, conf, "WINECORD_GATEWAY");
    ...
    /* default callbacks */
    gw->scheduler = _winecord_on_scheduler_default;
    ...
    winecord_gateway_send_presence_update(gw, gw->id.presence);
}

void
winecord_gateway_cleanup(struct winecord_gateway *gw)
{
    if (gw->timer->hbeat_timer)
        winecord_internal_timer_ctl(CLIENT(gw, gw),
                                   &(struct winecord_timer){
                                       .id = gw->timer->hbeat_timer,
                                       .flags = WINECORD_TIMER_DELETE,
                                   });
...
static bool
_winecord_gateway_session_from_json(struct winecord_gateway_session *session,
                                   const char text[],
                                   size_t len)
{
    jsmn_parser parser;
    ...
    if ((f = jsmnf_find(pairs, text, "url", 3))) {
        ...
        url_len = snprintf(session->base_url, sizeof(session->base_url),
                           "%.*s%s" WINECORD_GATEWAY_URL_SUFFIX, url_len, url,
                           ('/' == url[url_len - 1]) ? "" : "/");
    ...
    if ((f = jsmnf_find(pairs, text, "session_start_limit", 19)))
        winecord_session_start_limit_from_jsmnf(f, text, &session->start_limit);
    return true;
}

WINEBERRY
winecord_gateway_start(struct winecord_gateway *gw)
{
    struct ccord_szbuf json = { 0 };

    if (gw->session->retry.attempt == gw->session->retry.limit) {
        logconf_fatal(&gw->conf,
                      "Failed reconnecting to Winecord after %d tries",
                      gw->session->retry.limit);

        return WINEBERRY_DISCORD_CONNECTION;
    }

    if (winecord_get_gateway_bot(CLIENT(gw, gw), &json) != WINEBERRY_OK
        || !_winecord_gateway_session_from_json(gw->session, json.start,
                                               json.size))
    {
        logconf_fatal(&gw->conf, "Couldn't retrieve Gateway Bot information");
        free(json.start);

        return WINEBERRY_DISCORD_BAD_AUTH;
    }
    free(json.start);
    ...
    if (!gw->session->start_limit.remaining) {
        ...
        return WINEBERRY_DISCORD_RATELIMIT;
    }
...
bool
winecord_gateway_end(struct winecord_gateway *gw)
{
    ws_end(gw->ws);
    ...
    if (!gw->session->retry.enable) {
        logconf_warn(&gw->conf, "Winecord Gateway Shutdown");

        /* reset for next run */
        gw->session->status = WINECORD_SESSION_OFFLINE;
        gw->session->is_ready = false;
        gw->session->retry.enable = false;
        gw->session->retry.attempt = 0;
...
WINEBERRY
winecord_gateway_perform(struct winecord_gateway *gw)
{
    switch (ws_get_status(gw->ws)) {
    case WS_CONNECTING:
    case WS_CONNECTED:
        if (ws_multi_socket_run(gw->ws, &gw->timer->now)) return WINEBERRY_OK;
    /* fall-through */
    default:
        return WINEBERRY_DISCORD_CONNECTION;
    }
...
void
winecord_gateway_shutdown(struct winecord_gateway *gw)
{
...
void
winecord_gateway_reconnect(struct winecord_gateway *gw, bool resume)
{
